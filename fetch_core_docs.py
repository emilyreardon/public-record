...
OUT_DIR = os.path.join(os.getcwd(), "new-public-systems-corpus", "_raw_core")
...
PAGES = [
    ("https://www.archives.gov/founding-docs/declaration-transcript", "declaration_raw.html"),
    ("https://www.archives.gov/founding-docs/constitution-transcript", "constitution_raw.html"),
    ("https://www.archives.gov/founding-docs/bill-of-rights-transcript", "amendments_1-10_raw.html"),
    ("https://www.archives.gov/founding-docs/amendments-11-27", "amendments_11-27_raw.html"),
]
...
def diagnose():
    ...
    try:
        r = requests.get("https://www.archives.gov", timeout=TIMEOUT)
        log("archives.gov reachable: HTTP " + str(r.status_code))
        return True
    except Exception as e:
        log("FAILED to reach archives.gov: " + repr(e))
        return False
...
def main():
    if not diagnose():
        sys.exit(1)
    log("=== fetching core docs into " + OUT_DIR + " ===")
    results = []
